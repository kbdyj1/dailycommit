...
BEGIN_MESSAGE_MAP(CMFCApplication1App, CWinAppEx)
	...
	ON_COMMAND(ID_FILE_OPEN, &CMFCApplication1App::OnFileOpen)
	// 표준 인쇄 설정 명령입니다.
...
void CMFCApplication1App::OnFileOpen()
{
	wchar_t filter[] = L"All Files (*.*)|*.*|Image Files (*.jpg)|*.jpg||";
	CFileDialog fileDlg(TRUE, L"Image", L"*.jpg", OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_NOCHANGEDIR, filter, NULL);
	if (fileDlg.DoModal() == IDOK) {
		// 선택된 파일의 경로명을 이용하여 도큐먼트 정보를 재구성한다.
		CString imagePath = fileDlg.GetPathName();
		CString message = imagePath;
		message += " is selected.";

		CMainFrame* pMainFrame = dynamic_cast<CMainFrame*>(AfxGetApp()->GetMainWnd());
		if (pMainFrame) {
			CMFCApplication1Doc* pDoc = dynamic_cast<CMFCApplication1Doc*>(pMainFrame->GetActiveDocument());
			if (pDoc) {
				OutputDebugString(message);

				pDoc->SetImage(imagePath);
				CMFCApplication1View* pView = dynamic_cast<CMFCApplication1View*>(pMainFrame->GetActiveView());
				if (pView) {
					pView->Invalidate();
				}

				pMainFrame->SetStatusBarText(imagePath);
			}
		}
	}
}
